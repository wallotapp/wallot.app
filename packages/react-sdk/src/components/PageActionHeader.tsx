...
export const PageActionHeader: React.FC<PageActionHeaderProps> = ({ className = '' }) => {
	// Site Origin by Target
	const siteOriginByTarget = useSiteOriginByTarget();
	return (
		<div className={cn('h-10 items-center justify-between px-6 fixed top-12 left-0 w-full z-10', 'lg:px-28', 'bg-brand-dark', 'hidden md:flex', className)}>
			<div className='flex items-center space-x-1'>
				<div className={cn('flex items-center space-x-2', 'bg-black py-0.5 px-2.5 rounded-full')}>
					<p className='font-light text-xs text-white'>New</p>
				</div>
				<div>
					<p className='font-light text-xs text-white'>Introducing Wallot's brand new digital trading platform powered by AI</p>
				</div>
			</div>
