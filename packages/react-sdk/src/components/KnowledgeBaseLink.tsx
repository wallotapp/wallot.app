...
export const KnowledgeBaseLink: React.FC<BaseComponent> = ({ className = '' }) => {
	const {
		routeState: { currentRouteStaticId },
	} = useRouteStateContext();
	const siteOriginByTarget = useSiteOriginByTarget();
	const knowledgeBaseWebAppOrigin = siteOriginByTarget['KNOWLEDGE_BASE_WEB_APP'];
	...
	const knowledgeBaseTarget = SITE_ORIGIN !== knowledgeBaseWebAppOrigin ? '_blank' : '';
	return (
		<Link className={className} href={knowledgeBaseHref} target={knowledgeBaseTarget}>
			<p className={cn('font-light text-sm', currentRouteStaticId?.startsWith('KNOWLEDGE_BASE_WEB_APP') && 'underline underline-offset-4')}>Learn</p>
		</Link>
	);
};
