...
export const getAssetPropertiesFromAlpacaAsset = (alpacaAsset: AlpacaAsset): Pick<Asset, AlpacaAssetPropertyName> => {
	return AlpacaAssetPropertyNameEnum.arr.reduce((acc, propertyName) => {
		const originalPropertyName = propertyName.replace('alpaca_asset_', '') as keyof AlpacaAsset;
