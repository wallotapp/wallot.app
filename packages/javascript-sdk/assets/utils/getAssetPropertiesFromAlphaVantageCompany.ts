...
export const getAssetPropertiesFromAlphaVantageCompany = (alphaVantageCompany: AlphaVantageCompany): Pick<Asset, AlphaVantageCompanyPropertyName> => {
	return AlphaVantageCompanyPropertyNameEnum.arr.reduce((acc, propertyName) => {
		const originalPropertyName = propertyName.replace('alpha_vantage_company_', '') as keyof AlphaVantageCompany;
