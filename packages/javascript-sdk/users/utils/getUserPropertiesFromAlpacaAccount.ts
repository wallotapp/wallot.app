...
export const getUserPropertiesFromAlpacaAccount = (alpacaAccount: AlpacaAccount): Pick<User, AlpacaAccountPropertyName> => {
	return AlpacaAccountPropertyNameEnum.arr.reduce((acc, propertyName) => {
		const originalPropertyName = propertyName.replace('alpaca_account_', '') as keyof AlpacaAccount;
