...
export const locateBestModelFamiliesForParameterSet = async ({
	...
	try {
		...
		const modelFamilies = (
			await db
				.collection(modelFamiliesApi.collectionId)
				.where('parameters', 'array-contains-any', parameterIds)
				.get()
		).docs.map((doc) => doc.data() as ModelFamily);
		...
		return modelFamilies.sort((a, b) => {
			const aMatches = a.parameters.filter((parameter) =>
				parameterIds.includes(parameter),
			).length;
			const bMatches = b.parameters.filter((parameter) =>
				parameterIds.includes(parameter),
			).length;
			return bMatches - aMatches;
		});
	} catch (err) {
		const modelFamiliesWithoutFilter = await db
			.collection(modelFamiliesApi.collectionId)
			.limit(1)
			.get();
